...
The `home-routes` handler in the `{{name}}.routes.home` namespace
...
The `home-page` function will in turn call the `{{name}}.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `{{name}}.handler` namespace:
...
Middleware used for development is placed in the `{{name}}.dev-middleware` namespace found in
